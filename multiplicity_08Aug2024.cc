...
int caf_plotter(std::string input_file_list, std::string output_rootfile) {
    ...
    TH1D *reco_mult_prim_total_bilal = new TH1D("reco_mult_prim_total_bilal", "Reconstructed Track Multplicity;Multiplicity;Counts", 20, 0, 20);
    ...
    for (long n = 0; n < Nentries; ++n) { 
        ...
        for (long unsigned ntrue = 0; ntrue < sr->mc.nu.size(); ntrue++) {
			...
			if (abs(sr->mc.nu[ntrue].vtx.x) > maxX || abs(sr->mc.nu[ntrue].vtx.x) < minX) continue;
 		    if (abs(sr->mc.nu[ntrue].vtx.z) > maxZ || abs(sr->mc.nu[ntrue].vtx.z) < minZ) continue;
		    if (abs(sr->mc.nu[ntrue].vtx.y) > maxY || abs(sr->mc.nu[ntrue].vtx.y) < minY) continue; 
            ...
            for (long unsigned primaries = 0; primaries < truePrimary.size(); primaries++) {
				...
				// bool isXOutOfRange_true = abs(currentX_prim_true) > 53 || abs(currentX_prim_true) < 12;
                // bool isYOutOfRange_true = abs(currentY_prim_true) > 53;
                // bool isZOutOfRange_true = abs(currentZ_prim_true) < 53 || abs(currentZ_prim_true) > 12;
				...
				// if (isXOutOfRange_true || isYOutOfRange_true || isZOutOfRange_true) {
                    // continue; // Skip to the next iteration if any condition is true
                // }
        ...
        for (long unsigned nixn = 0; nixn < sr->common.ixn.dlp.size(); nixn++) {
			...
			int recoPrimary_total_bilal = 0;		
			// Loop over reconstructed particles
            for (long unsigned npart = 0; npart < sr->common.ixn.dlp[nixn].part.dlp.size(); npart++) {
				auto& pdg = sr->common.ixn.dlp[nixn].part.dlp[npart].pdg;
				
				if ((abs(pdg)==13 || abs(pdg)==2212 || abs(pdg)==211 || abs(pdg)==321)){
					 recoPrimary_total_bilal++;
				}	
			}		
			
			if (recoPrimary_total_bilal > 0) {
				reco_mult_prim_total_bilal->Fill(recoPrimary_total_bilal);
			}
		}
...
reco_mult_prim_total_bilal->Write();

    SaveHistogramAsPNG(true_genie_multiplicity);    	
	SaveHistogramAsPNG(reco_mult_prim_total_bilal);
	// DrawAndSaveHistogramWithWeightedStats_onehistogram(true_genie_multiplicity,"Multiplicity;Number of Charged Tracks;Counts", "true_genie_multiplicity_weighted.png");    
